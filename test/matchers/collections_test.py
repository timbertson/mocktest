...
class CollectionsTest(TestCase):
	...
	def test_should_match_multuple_items_in_a_collection(self):
		self.assertTrue(object_containing('f', 'g').matches(Collection(['a','g', 'b','f'])))
		self.assertTrue(object_containing('f', 'g').matches(Collection(['a','b','f'])))
	...
	def test_should_match_a_dictionary_with_value_matchers(self):
		...
		self.assertTrue(dict_containing(x=Any(int)).matches({'x':'three'}))
